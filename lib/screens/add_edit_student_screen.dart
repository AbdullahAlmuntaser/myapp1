    ...
    if (mounted) {
      setState(() {
        ...
        if (_selectedParentUserId != null) {
          final selectedParent = _parents.firstWhere((p) => p.id == _selectedParentUserId, orElse: () => _parents.first);
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Failed to save student: ${e.toString()}'),
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  validator: (value) => value!.isEmpty ? 'Please enter an academic number' : null,
                ),
                const SizedBox(height: 16),
                TextFormField(
                ...
                DropdownButtonFormField<int>(
                  ...
                  items: _parents.map((User parentUser) {
                    return DropdownMenuItem<int>(
                      value: parentUser.id,
                      child: Text('${parentUser.username} (${parentUser.role})'),
                    );
                  }).toList(),
                  onChanged: (int? newValue) {
                    setState(() {
                      ...
                      if (newValue != null) {
                        final selectedParent = _parents.firstWhere((p) => p.id == newValue);
                        _parentNameController.text = selectedParent.username;
                        _parentPhoneController.text = selectedParent.phone ?? '';
                      } else {
