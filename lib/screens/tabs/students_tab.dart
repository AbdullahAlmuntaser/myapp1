...
class StudentsTab extends StatefulWidget {
  final int? parentUserId; // New optional parameter

  const StudentsTab({super.key, this.parentUserId});
...
class StudentsTabState extends State<StudentsTab> {
  ...
  Future<void> _fetchInitialData() async {
    ...
    try {
      ...
      if (widget.parentUserId != null) {
        await studentProvider.fetchStudentsByParentUserId(widget.parentUserId!);
      } else {
  ...
  void _filterStudents() async {
    ...
    try {
      final studentProvider = Provider.of<StudentProvider>(context, listen: false);
      if (widget.parentUserId != null) {
        // If parentUserId is provided, filter from parent's students
        await studentProvider.searchStudentsByParentUserId(
          widget.parentUserId!,
          _searchController.text,
          classId: _selectedClassId,
        );
      } else {
        // Otherwise, search all students
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(widget.parentUserId != null ? 'أبنائي' : 'لوحة تحكم الطلاب'),
                ...
                : Consumer<StudentProvider>(
                    builder: (context, studentProvider, child) {
                      ...
                      return isLargeScreen
                          ? _buildWebLayout(studentProvider.students, currentUserRole)
                          : _buildMobileLayout(studentProvider.students, currentUserRole);
                    },
                  ),
          ),
        ],
      ),
      floatingActionButton: (currentUserRole == 'admin' || currentUserRole == 'teacher') && widget.parentUserId == null
          ? FloatingActionButton(
              onPressed: _isLoading ? null : () => _navigateToAddEditScreen(),
              tooltip: 'إضافة طالب جديد',
              child: const Icon(Icons.add),
            )
          : null, // Hide FAB for parents and students
    );
  }

  Widget _buildMobileLayout(List<Student> students, String? currentUserRole) {
      ...
      itemBuilder: (context, index) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            trailing: (currentUserRole == 'admin' || currentUserRole == 'teacher')
                ? PopupMenuButton<String>(
                    ...
                    itemBuilder: (context) => [
                      PopupMenuItem(enabled: !_isLoading, value: 'edit', child: const Text('تعديل')),
                      PopupMenuItem(enabled: !_isLoading, value: 'delete', child: const Text('حذف')),
                    ],
                  )
                : null, // Hide actions for parents and students
          ),
        );
      },
    );
  ...
  Widget _buildWebLayout(List<Student> students, String? currentUserRole) {
    return SingleChildScrollView(
      scrollDirection: Axis.vertical,
      child: DataTable(
        columns: [
          const DataColumn(label: Text('الاسم')),
          const DataColumn(label: Text('الرقم الأكاديمي')),
          const DataColumn(label: Text('البريد الإلكتروني')),
          const DataColumn(label: Text('الصف')),
          const DataColumn(label: Text('الفصل')),
          const DataColumn(label: Text('ولي الأمر')),
