...
class DatabaseHelper {
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    ...
    try {
      ...
      if (oldVersion < 16) { // New version for attendance table alteration
        await db.execute('ALTER TABLE attendance ADD COLUMN lateMinutes INTEGER');
      }
      if (oldVersion < 17) { // New version for grades table alteration
        await db.execute('ALTER TABLE grades ADD COLUMN date TEXT NOT NULL DEFAULT CURRENT_DATE');
        await db.execute('ALTER TABLE grades ADD COLUMN description TEXT');
        await db.execute('ALTER TABLE grades ADD COLUMN maxGrade REAL');
      }
      if (oldVersion < 18) {
        await db.execute('''
          CREATE TABLE assessment_types(
  ...
  Future<String?> getLatestAcademicYear() async {
    final db = await database;
    final List<Map<String, dynamic>> result = await db.rawQuery('''
      SELECT MAX(yearTerm) as latestYear FROM classes
    ''');
    if (result.isNotEmpty && result.first['latestYear'] != null) {
      return result.first['latestYear'] as String;
    }
    return null; // No academic year found
  }
  ...
  Future<List<Grade>> getGradesByStudentAndYear(int studentId, String yearTerm) async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.rawQuery('''
      SELECT g.*
      FROM grades g
      JOIN classes c ON g.classId = c.id
      WHERE g.studentId = ? AND c.yearTerm = ?
    ''', [studentId, yearTerm]);
    return List.generate(maps.length, (i) {
  ...
  Future<List<Grade>> getGradesByClassAndYear(int classId, String yearTerm) async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.rawQuery('''
      SELECT g.*
      FROM grades g
      JOIN classes c ON g.classId = c.id
      WHERE g.classId = ? AND c.yearTerm = ?
    ''', [classId, yearTerm]);
    return List.generate(maps.length, (i) {
      return Grade.fromMap(maps[i]);
    });
  ...
  Future<List<Map<String, dynamic>>> getAttendanceSummaryByDate() async {
    final db = await database;
    final List<Map<String, dynamic>> result = await db.rawQuery('''
      SELECT
        date,
  ...
  Future<int> getTotalTeachersCount() async {
    final db = await database;
    final count = Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM teachers'));
    return count ?? 0;
  }

  Future<int> getTotalClassesCount() async {
    final db = await database;
    final count = Sqflite.firstIntValue(await db.rawQuery('SELECT COUNT(*) FROM classes'));
    return count ?? 0;
  }
  ...
  Future<void> clearAllData() async {
    ...
    await db.delete('assessment_types');
  }

  // AssessmentType CRUD
  Future<int> createAssessmentType(AssessmentType assessmentType) async {
    final db = await database;
    return await db.insert('assessment_types', assessmentType.toMap());
  }

  Future<List<AssessmentType>> getAssessmentTypes() async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.query('assessment_types');
    return List.generate(maps.length, (i) {
      return AssessmentType.fromMap(maps[i]);
    });
  }

  Future<int> updateAssessmentType(As